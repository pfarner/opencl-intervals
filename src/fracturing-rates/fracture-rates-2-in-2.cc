...
void writeDemo(std::ostream& os, int iteration, int iterations, const Intervals<N>& intervals) {
  ...
  os << "set x2label \"subdividing ranges to find 2D solutions in a 2D space: (x-0.3)²+(y-0.7)²≤0.1²\\niteration " << iteration
     << ": " << count << " prisms"
...
int main(void) {
  try {
    Subdivider<2> subdivider(Kernel::read("2-in-2.cl"));
    ...
    os << "set output '2-in-2.gif~'" << std::endl;
    ...
    os << "system \"mv 2-in-2.gif~ 2-in-2.gif\"" << std::endl;
  } catch(string str) {
