...
void writeDemo(std::ostream& os, int iteration, int iterations, const Intervals<N>& intervals) {
  ...
  os << "set view 75,30,1,1" << std::endl;
  os << "set title \"subdividing ranges to find 2D solutions in a 3D space: (x-0.3)²+(y-0.5)²+(z-0.7)²=0.1² or z=x²y\\niteration " << iteration
     << ": " << count << " prisms"
...
int main(void) {
  try {
    Subdivider<3> subdivider(Kernel::read("2-in-3.cl"));
    ...
    os << "set output '2-in-3.gif~'" << std::endl;
    ...
    os << "system \"rsync -ac 2-in-3.gif~ 2-in-3.gif\"" << std::endl;
  } catch(string str) {
