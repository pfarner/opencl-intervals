...
void writeDemo(std::ostream& os, int iteration, int iterations, const Intervals<N>& intervals) {
  ...
  os << "set title \"subdividing ranges to find 0D solutions in a 3D space: x*3==int(x*3) and y*5==int(y*5) and z*5==int(z*5) and y≤x and z≤x+y\\niteration " << iteration
     << ": " << count << " prisms"
...
int main(void) {
  try {
    Subdivider<3> subdivider(Kernel::read("0-in-3.cl"));
    ...
    os << "set output '0-in-3.gif~'" << std::endl;
    ...
    os << "system \"rsync -ac 0-in-3.gif~ 0-in-3.gif\"" << std::endl;
  } catch(string str) {
