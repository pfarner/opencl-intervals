...
void writeDemo(std::ostream& os, int iteration, int iterations, const Intervals<N>& intervals) {
  ...
  os << "set x2label \"subdividing ranges to find 1D solutions in a 2D space: (x-0.3)²+(y-0.7)²=0.1² or y=x³\\niteration " << iteration
     << ": " << count << " prisms"
     << ", covering total area of " << totalVolume
     << ", next subdivision is in " << (intervals.phase==0 ? "X" : "Y") << " direction"
     << "\"" << std::endl;
  os << "plot"
     << " 0.7 + sqrt(0.1**2 - (x-0.3)**2) w l lt 1 lw 3 t '',"
     << " 0.7 - sqrt(0.1**2 - (x-0.3)**2) w l lt 1 lw 3 t '',"
     << " x**3                            w l lt 2 lw 3 t ''"
     << std::endl;
...
int main(void) {
  try {
    Subdivider<2> subdivider(Kernel::read("1-in-2.cl"));
    ...
    os << "system \"mv 1-in-2.gif~ 1-in-2.gif\"" << std::endl;
  } catch(string str) {
